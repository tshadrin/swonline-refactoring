...
if ( 0 < $player_room )
{
    ...
    if ( $owner_id == $player_clan && $owner_typ == 1 )
    {
        if ( $mp[$dir] != "" && $do == "build" && $map[$dir] == 0 )
        {
            ...
            sql_do( $SQL );
            $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat,weight) values ({$max},'Общий склад','sunduk','Сундук',3,{$player_clan},NOW(),7)";
            sql_do( $SQL );
            $player['room'] = 0;
        ...
        $t = "";
        $t .= "<table><tr><td><table><tr><td width=75 align=center><img src=pic/stuff/else/gid.gif></td><td><b>Название создаваемого склада:</b> `Склад [{$litle}]`<br><br>Выберите направление строительства склада относительно вашей текущей комнаты.</td></tr></table>";
