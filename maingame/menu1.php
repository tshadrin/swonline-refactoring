...
else if ($load == 'exit')
{
	...
	$SQL="select id,start_room,end_room from sw_arena where typ=1";
		...
		mysqli_free_result($result);
	if ($er == 0)
		print "<script>top.ttext('Выход из игры','<table width=100% height=280><tr><Td align=center><b>Выход из игры произойдёт автоматически через 30 секунд.$rg</b></td></tr></table>');ExitTimer = setTimeout('document.location=\'menu.php?load=exit2\'',30000);</script>";
	else
	...
	if ($city_name <> "")
	{
		...
		if ($action == 5)
		{
			...
			if  ($city_rank == 1)
			{
				...
				if ($city_dead == 0)
					$info .= "<tr bgcolor=F7FBFF><form action=menu.php target=menu><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=hidden name=do value=adddead><td  align=center><b>Усыпальница<br> с регенирацией</b></td><td  align=center>0</td><td  align=center>5000 / 200</td><td align=center><input type=submit value=Купить style=width:70></td></form></tr>";
				else
					$info .= "<tr bgcolor=F7FBFF><form action=menu.php target=menu><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=hidden name=do value=deldead><td  align=center><b>Усыпальница<br> с регенирацией</b></td><td  align=center>1</td><td  align=center>5000 / 200</td><td align=center><input type=submit value=Разрушить style=width:70></td></form></tr>";
				$info .= "<tr bgcolor=F7FBFF><td  align=center><b>Стража</b></td><td  align=center>$city_npc</td><td  align=center>1000 / 100</td><td align=center><table cellpadding=0 cellspacing=0><tr><form action=menu.php target=menu><TD><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=hidden name=do value=addguard><input type=submit value=Нанять style=width:70></td></form></tr><tr><td><form action=menu.php target=menu><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=hidden name=do value=delguard><input type=submit value=Уволить style=width:70></td></form></tr></table></td></tr>";
			}
			else
			{
...
//						print "$objname|$isshop";
							...
							if ($objcount + $count <= 11000)
							{
								...
								if ($plid > 0)
								{
									...
									else
									{
										...
										$info .= "<tr><form action=menu.php method=post target=menu><input type=hidden name=toplace value=$toplace><input type=hidden name=do3 value=extend><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=hidden name=do value=$do><input type=hidden name=do2 value=next><td><b>&nbsp;&nbsp;Добавить предмет:</b></td><td><input type=hidden name=objname value=\"$objname\">$objname</td></tr>";
										...
										$info .= "<tr><td colspan=2 align=center><input type=hidden name=isshop value=\"$isshop\"><input type=\"submit\" value=\"Купить товар\"></td><td></td></form></tr>";
										$info .= "</table>";
										$no_drow = 1;
									}
								}
				...
				if ($no_drow == 0)
				{
					...
					$text = "Магазин города <b>$city_name</b>";
					$menu .= "<br><font color=AA0000><b>» Экономика</b></font>";
					$info = "<table cellpadding=2 width=100%><tr><Td colspan=2><b><font color=AAAAAA>- Добавить предмет в магазин</font></b></td></tr>";
					$info .= "<tr><td><b>&nbsp;&nbsp;Золота в казне:</b></td><td>$city_money злт.</td></tr>";
					$info .= "<tr><form action=menu.php method=post target=menu><input type=hidden name=load value=$load><input type=hidden name=toplace value=$mag_id><input type=hidden name=action value=$action><input type=hidden name=do value=$do><input type=hidden name=isshop value=yes><input type=hidden name=do2 value=next><td><b>&nbsp;&nbsp;Добавить предмет:</b></td><td>$select</td></tr>";
					$info .= "<tr><td><b>&nbsp;&nbsp;Количество предметов:</b></td><td><input type=text name=objcount size=6 maxlength=6 value=1></td></tr>";
					$info .= "<tr><td colspan=2 align=center><input type=\"submit\" value=\"Предварительный просмотр\"></td><td></td></form></tr>";
					$info .= "</table>";

					$info .= "<table cellpadding=2 width=100%><tr><Td colspan=2><b><font color=AAAAAA>- Добавить предмет в библиотеку</font></b></td></tr>";
					$info .= "<tr><td><b>&nbsp;&nbsp;Золота в казне:</b></td><td>$city_money злт.</td></tr>";
					$info .= "<tr><form action=menu.php method=post target=menu><input type=hidden name=load value=$load><input type=hidden name=toplace value=$bib_id><input type=hidden name=action value=$action><input type=hidden name=do value=$do><input type=hidden name=do2 value=next><td><b>&nbsp;&nbsp;Добавить предмет:</b></td><td>$select2</td></tr>";
					$info .= "<tr><td><b>&nbsp;&nbsp;Количество предметов:</b></td><td><input type=text name=objcount size=6 maxlength=6 value=1></td></tr>";
					$info .= "<tr><td colspan=2 align=center><input type=\"submit\" value=\"Предварительный просмотр\"></td><td></td></form></tr>";
					$info .= "</table>";
				}
			...
			else
			{
				...
				if  ($city_rank == 1)
				{
					$info = "<table cellpadding=1 width=100%><tr><Td colspan=2><b><font color=AAAAAA>- Налоги</font></b></td></tr><tr><td><b>&nbsp;&nbspНалог с покупок</b></td><form action=menu.php method=post target=menu><td class=usergood width=130><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=text name=ccity_buy value=$city_buy size=3 maxlength=3>&nbsp;<input type=submit value=Изменить></td></form></tr><tr><td><b>&nbsp;&nbspНалог с продаж</b></td><form action=menu.php method=post target=menu><td class=usergood><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=text name=ccity_sell value=$city_sell size=3 maxlength=3>&nbsp;<input type=submit value=Изменить></td></form></tr><tr><td><b>&nbsp;&nbspНалог с починок</b></td><form action=menu.php method=post target=menu><td class=usergood><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=text name=ccity_rep value=$city_rep size=3 maxlength=3>&nbsp;<input type=submit value=Изменить></td></form></tr><tr><td><b>&nbsp;&nbspБанковский налог</b></td><form action=menu.php method=post target=menu><td class=usergood><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=text name=ccity_bank value=$city_bank size=3 maxlength=3>&nbsp;<input type=submit value=Изменить></td></form></tr>";
					$info .= "<tr><td><b>&nbsp;&nbspПремия на арене</b></td><form action=menu.php method=post target=menu><td class=usergood><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=text name=ccity_fight value=$city_fight size=3 maxlength=3>&nbsp;<input type=submit value=Изменить></td></form></tr><tr><tr><td colspan=2></td></tr><tr><Td colspan=2><b><font color=AAAAAA>- Золотой запас</font></b></td></tr><tr><td colspan=2></td></tr><tr><td><b>&nbsp;&nbspЗолотой запас города</b></td><td class=usergood>$city_money <b>злт</b></td></tr><tr><td colspan=2></td></tr><tr><Td colspan=2><b><font color=AAAAAA>- Статистика</font></b></td></tr><tr><td colspan=2></td></tr><tr><td><b>&nbsp;&nbspМесто по финансам</b></td><td class=usergood>$place место</td></tr><tr><td colspan=2><b>&nbsp;&nbsp;</b><a href=menu.php?load=city&action=6&do=admshop target=menu class=menu2><b>» Перейти к управление магазином<b></a></td></tr></table>";
				}
				else
		...
		if ($action == 11)
		{
			...
			$r = rand(0,999999);
			$info .= "<table cellpadding=4><form action=menu.php method=post target=menu><input type=hidden name=action value=$action><input type=hidden name=load value=$load><input type=hidden name=do value=del><tr><td colspan=3><b><font color=AAAAAA>- Выгнать жителя города.</a></b></td></tr><td><b>Имя персонажа:</b> </td><td><input type=text name=name value=\"$name\" size=10></td><td><input type=submit value=Выгнать></td></tr></form></table>";
			if ($do == "del")
			{
				...
				if ($nname <> "")
				{
					if (($c_rank == 1) && ($city_rank <> 1))
						...
						$info .= "<table cellpadding=4><form action=menu.php method=post target=menu><input type=hidden name=plid value=$id><input type=hidden name=action value=$action><input type=hidden name=load value=$load><input type=hidden name=do value=del2><tr><td><b>Пользователь найден:</b> $nname <i>$level уровень.</i></td></tr><tr><td>Причина:<br><div align=center><textarea cols=50 rows=5 name=for></textarea><br><input type=submit value=Выгнать></div></td></tr></form></table>";
				}
			}
			else if  ($do == "del2")
			{
	...
	if (($put_money > 0) && ($myclan == $city_id))
	{
		if (($player_legs == 1) && ( $player_leg == 1))
		{
			...
			if ($gold - $put_money  >= 0)
			{
				...
				SQL_do($SQL);
				$SQL="update sw_users set gold=gold-$put_money where id=$player_id";
				SQL_do($SQL);
	...
	if ($city_name <> "")
	{
		...
		if ($city_id == $myclan)
		{
			...
			if ($action == 6)
			{
				...
				{
					if ($do <> "showlog")
						$info = "<table cellpadding=3 width=100%><tr><Td colspan=2><b><font color=AAAAAA>- Золотой запас</font></b></td></tr><tr><td colspan=2></td></tr><tr><td><b>&nbsp;&nbspВзять золото</b></td><form action=menu.php method=post target=menu><td class=usergood width=130><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=text name=get_money value=1 size=5 maxlength=5>&nbsp;<input type=submit value=Забрать></td></form></tr><tr><td><b>&nbsp;&nbspПополнить казну</b></td><form action=menu.php method=post target=menu><td class=usergood><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=text name=put_money value=1 size=5 maxlength=5>&nbsp;<input type=submit value=Пополнить></td></form></tr><tr><td colspan=2></td></tr><tr><Td colspan=2><b><font color=AAAAAA>- Информация</font></b></td></tr><tr><td colspan=2></td></tr><tr><td><b>&nbsp;&nbspЗолотой запас клана</b></td><td class=usergood>$city_money <b>злт</b></td></tr><tr><td colspan=2></td></tr><tr><Td colspan=2><b><font color=AAAAAA>- Статистика</font></b></td></tr><tr><td colspan=2></td></tr><tr><td><b>&nbsp;&nbspМесто по финансам</b></td><td class=usergood>$place место</td></tr><tr><td colspan=2><br><a href=menu.php?load=clan&action=6&do=showlog&sh=0 target=menu class=menu2><b>&nbsp;» Посмотреть лог казны<b></a><br><a href=menu.php?load=clan&action=6&do=showlog&sh=1 target=menu class=menu2><b>&nbsp;» Посмотреть лог магазина<b></a></td></tr></table>";
					else
					{
					...
					$info = "<table cellpadding=3 width=100%><tr><Td colspan=2><b><font color=AAAAAA>- Золотой запас</font></b></td></tr><tr><td><b>&nbsp;&nbspПополнить казну</b></td><form action=menu.php method=post target=menu><td class=usergood><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=text name=put_money value=1 size=5 maxlength=5>&nbsp;<input type=submit value=Пополнить></td></form></tr><tr><td colspan=2></td></tr><tr><Td colspan=2><b><font color=AAAAAA>- Информация</font></b></td></tr><tr><td colspan=2></td></tr><tr><td><b>&nbsp;&nbspЗолотой запас клана</b></td><td class=usergood>$city_money <b>злт</b></td></tr><tr><td colspan=2></td></tr><tr><Td colspan=2><b><font color=AAAAAA>- Статистика</font></b></td></tr><tr><td colspan=2></td></tr><tr><td><b>&nbsp;&nbspМесто по финансам</b></td><td class=usergood>$place место</td></tr></table>";
			}
			else
		...
		if ($action == 11)
		{
			...
			$menu .= "<br><font color=AA0000><b>» Жители</b></font>";
			$info .= "<table cellpadding=4><form action=menu.php method=post target=menu><input type=hidden name=action value=$action><input type=hidden name=load value=$load><input type=hidden name=do value=add><tr><td colspan=3><b><font color=AAAAAA>- Принять в клан.</a></b></td></tr><td><b>Имя персонажа:</b> </td><td><input type=text name=name2 value=\"$name2\" size=10></td><td><input type=submit value=Принять></td></tr></form></table>";
			if ($do == "add")
			{
			...
			if ($city_rank == 1)
			{
				$info .= "<table cellpadding=4><form action=menu.php method=post target=menu><input type=hidden name=action value=$action><input type=hidden name=load value=$load><input type=hidden name=do value=del><tr><td colspan=3><b><font color=AAAAAA>- Выгнать из клана.</a></b></td></tr><td><b>Имя персонажа:</b> </td><td><input type=text name=name value=\"$name\" size=10></td><td><input type=submit value=Выгнать></td></tr></form></table>";
		...
		if ($action == 20)
		{
			if ($do == "take")
			{
				...
				$SQL="update sw_users SET exp=exp-50 where id=$player_id";
				SQL_do($SQL);
				$mtext = "<b>* Опыт -50 *</b>";
			...
			if ($is_ring == 0)
				$info .= "</table></td></tr><tr><td colspan=2 align=center>$tm</td></tr><tr><form action=menu.php method=post target=menu><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=hidden name=city_id value=$city_id><input type=hidden name=do value=take><input type=hidden name=load value=$load><td colspan=2 align=center><br><input type=submit value=\'Взять такое кольцо (-50 опыта).\' style=width:80%></td></tr></form></table>";
			else
				$info .= "</table></td></tr><tr><td colspan=2 align=center>$tm</td></tr><tr><form action=menu.php method=post target=menu><input type=hidden name=load value=$load><input type=hidden name=action value=$action><input type=hidden name=city_id value=$city_id><input type=hidden name=do value=take><input type=hidden name=load value=$load><td colspan=2 align=center><br><input type=submit value=\'Взять такое кольцо (-50 опыта).\' style=width:80% disabled></td></tr></form></table>";
		}
		else
...
else if ($load == 'ignor')
{
	...
	$link = "<table cellspacing=3 border=0 width=300>";
		$link .= "<tr><form action=menu.php method=post target=menu><input type=hidden name=load value=$load><input type=hidden name=do value=add><tD><font color=AAAAAA><b>- Добавить</b></Font></td><td align=right><input type=text name=name size=14> <input type=submit value=Добавить></td></form></tr>";
