...
for ( ; $i < $count; $i = $i + 10 )
{
    ...
    {
        $p .= "|<a href=clannews.php?page={$i}&city_id={$city_id} class=menu>{$i}-{$e}</a>|";
    }
}
...
if ( $do == "add" && $allow == 1 )
{
    print "<form action=clannews.php method=post><input type=hidden name=do value=new><input type=hidden name=city_id value={$city_id}><tr bgcolor=D7DBDF><td><table width=100%><tr><td colspan=2 align=center><textarea cols=47 rows=4 name=text></textarea><br><table cellpadding=0 cellspacing=2><tr><td><input type=checkbox name=typ value=1></td><td> - Только для клана</td></tr></table>";
    print "<div align=center><input type=submit value='Отослать (1500 - символов)'></div>";
    print "</td></tr></table></td></tr></form>";
}
$i = 0;
$SQL = "select id,who,dat,tim,text,typ from sw_game_news where owner={$city_id} and city=0 {$sear} order by dat desc,tim desc limit {$page},10";
$row_num = sql_query_num( $SQL );
while ( $row_num )
{
    $id = $row_num[0];
